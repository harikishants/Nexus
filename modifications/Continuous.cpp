...
void Plot_Setting::setOutputDim(const std::string & x)
{
	outputDim.clear();
...
//Written by Hari for printing variables for each step and finding max, min of variables for all steps
void Plot_Setting::print_interval(const TaylorModelFlowpipes & flowpipes, Computational_Setting & setting, int current_step, double & min, double & max,bool print) const
{
	std::list<TaylorModelFlowpipe>::const_iterator fpIter = flowpipes.tmv_flowpipes.begin();
int step;
	unsigned int total_size = flowpipes.tmv_flowpipes.size();
	...
	double max_p; //max of previous step to check discontinuity in range propogation
	double min_p; 
	double max_c; //min of current
	double min_c;
	...
	if(total_size > 0)
	{
		...
		for(; fpIter != flowpipes.tmv_flowpipes.end() ; ++fpIter)
		{
			...
			TaylorModel<Real> tmTemp;
			outputDim.evaluate(tmTemp, fpIter->tmv_flowpipe.tms, order, newDomain, setting.tm_setting.cutoff_threshold, setting.g_setting);
			...
			if (min > X.inf()) min = X.inf();
			if (max < X.sup()) max = X.sup();
